(() => {
  ...
  function renderFocusStats() {
    ...
    if (host) { host.replaceChildren(); Object.entries(data.subjects).sort((a, b) => b[1] - a[1]).forEach(([name, value]) => { const row = document.createElement("div"); row.className = "focus-subject-row"; row.innerHTML = '<span>' + escText(name) + '</span><i style="width:' + (data.minutes ? value / data.minutes * 100 : 0) + '%"></i><small>' + value + ' 分钟</small>'; host.append(row); }); }
  }
  ...
  function finishRound() { const wasFocus = pomTimer.mode === "focus"; clearInterval(pomTimer.handle); pomTimer.handle = null; pomTimer.running = false; if (wasFocus) { record(Number(state.settings.focusMinutes || 25), true); pomTimer.mode = "break"; pomTimer.total = Number(state.settings.breakMinutes || 5) * 60; pomTimer.remaining = pomTimer.total; if (window.showToast) window.showToast("番茄完成，自动进入休息"); } else { pomTimer.mode = "focus"; pomTimer.total = Number(state.settings.focusMinutes || 25) * 60; pomTimer.remaining = pomTimer.total; if (window.showToast) window.showToast("休息结束，可以继续专注"); } customRenderFocus(); customRenderSchedule(); }
  function start() { if (pomTimer.running) return; const select = byId("focus-task-select"); if (select && select.value) pomTimer.taskId = select.value; if (!pomTimer.taskTitle) { const found = scheduleItemsFor(day()).find((item) => item.id === pomTimer.taskId) || state.tasks.find((item) => item.id === pomTimer.taskId); pomTimer.taskTitle = found && found.title || "未关联任务"; pomTimer.category = found && (found.type || found.category) || "未分类"; } pomTimer.startedAt = Date.now(); pomTimer.running = true; pomTimer.handle = setInterval(() => { pomTimer.remaining -= 1; if (pomTimer.remaining <= 0) finishRound(); else updateClock(); }, 1000); updateClock(); }
